...
module "server" {
  ...
  init_script_id       = ncloud_init_script.init_script.id     // see "init_script_id reference scenario" below
...
locals {
  servers = flatten([for server in var.servers : [
    for index in range(server.count) : merge(
      ...
      { additional_block_storages = [for vol in server.additional_block_storages : merge(
